...
$valid=$fnameErr=$lnameErr=$emailErr=$passErr=$cpassErr=$userErr='';
...
//Email Address Validation
 $conn = mysqli_connect("localhost", "root", "", "form_validation") or die("connection is failed");
...
if($fnameErr==1 && $lnameErr==1 && $emailErr==1 && $passErr==1 && $cpassErr==1)
{
   ...
   $valid="All fields are validated successfully and Recorded in DB";
   
   //legal input values
   $firstName= legal_input($first_name);
   $lastName=  legal_input($last_name);
   $email=     legal_input($email);
   $password1=  legal_input($password);
   ...
   // here you can write Sql Query to insert user data into database table
    ...
    header('localhost:validation-form.php');
